...
@implementation LBSecKeyWrapper

+ (SecKeyRef)addPublicKey:(NSString*)keyName keyString:(NSString*)keyString
{
    NSData* publicKey = [LBSecKeyWrapper stripPublicKeyHeader:[[NSData alloc] initWithBase64EncodedString:keyString options:NSDataBase64DecodingIgnoreUnknownCharacters]];
    return [LBSecKeyWrapper addPublicKey:keyName keyBits:publicKey];
...
//    LOGGING_FACILITY1( sanityCheck == noErr || sanityCheck == errSecDuplicateItem, @"Problem adding the app public key to the keychain, OSStatus == %ld.", sanityCheck );
    ...
    if (persistPeer)
    {
        peerKeyRef = [LBSecKeyWrapper getKeyRefWithPersistentKeyRef:persistPeer];
    }
    else
    {
...
+ (BOOL)verifySignature:(NSData *)plainText secKeyRef:(SecKeyRef)publicKey signature:(NSData *)sig
{
    ...
    sanityCheck = SecKeyRawVerify(    publicKey,
                                  kTypeOfSigPadding,
                                  (const uint8_t *)[[LBSecKeyWrapper getHashBytes:plainText] bytes],
                                  kChosenDigestLength,
...
+ (NSString *)shuffledAlphabet {
    ...
    for (NSUInteger i = 0; i < numberOfCharacters; ++i) {
        NSUInteger j = (arc4random_uniform((u_int32_t)(numberOfCharacters - i)) + i);
...
//    NSString* keyName = @"MeetingPublicKey";
    NSString* keyName = [NSString stringWithFormat:@"QH%@PublicKey",[self shuffledAlphabet]];
    OSStatus sanityCheck = noErr;
    SecKeyRef publicKeyRef = NULL;
    NSMutableData* cipherData = [NSMutableData data];

    publicKeyRef = [LBSecKeyWrapper addPublicKey:keyName keyBits:[LBSecKeyWrapper stripPublicKeyHeader:key]];
    ...
    int plainTextLen = (int)[plainText length];
    ...
    [LBSecKeyWrapper removePublicKey:keyName];
